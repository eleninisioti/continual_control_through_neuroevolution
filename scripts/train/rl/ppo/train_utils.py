...
class PPOExperiment(Experiment):
    ...
    def progress(self, info):
        ...
        ppo_params = env_params["params"]
        
        if not os.path.exists(self.config["exp_config"]["trial_dir"] + "/data/train/checkpoints_for_pca"):
            os.makedirs(self.config["exp_config"]["trial_dir"] + "/data/train/checkpoints_for_pca")
        
        #with open(self.config["exp_config"]["trial_dir"] + "/data/train/checkpoints_for_pca/ppo_params_" + str(gen) + ".pkl", "wb") as f:
