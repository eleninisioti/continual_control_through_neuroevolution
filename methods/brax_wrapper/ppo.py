...
def train(
    ...
    gravity_change_frequency: int = 30,  # ADD THIS LINE
  ...
  schedule_fn = optax.linear_schedule(
    ...
    end_value=learning_rate * 1.0,  # Decay to 10% of original
    transition_steps=num_timesteps
)
  ...
  for it in range(num_evals_after_init):
    ...
    for _ in range(max(num_resets_per_eval, 1)):
      ...
      if it % gravity_change_frequency == 0 and it:
        ...
        env, modified_xml_path = recreate_environment_with_gravity(
            env_name=env_params["env_name"],params= env_params["params"],
            gravity_multiplier=current_gravity, xml_file=env.robot.xml_file,
            save_file=True
        )
        eval_env = env
    ...
    if process_id == 0:
      ...
      # here lets do a PCA of PPO weights
