...
class GymnaxState(base.Base):
  ...
  info: Dict[str, tuple] = struct.field(default_factory=dict)
...
class VmapWrapper(Wrapper):
  ...
  def step(self, state: State, action: jax.Array, env_params) -> State:
    # gymnax step returns (obs, state, reward, done, info)
    # We need to extract the pipeline_state from the result
    key= jax.random.PRNGKey(0)
    def step_fn(state, action):
      state = self.env.step(key, state, action, env_params)
      return state
    
    return jax.vmap(step_fn)(state, action)
...
class EpisodeWrapper(Wrapper):
  ...
  def step(self, key, state: State, action: jax.Array, env_params) -> State:
    def f(state, _):
      # gymnax step returns (obs, state, reward, done, info)
      obs, gymnax_state, reward, done, _ = self.env.step(key, state.pipeline_state, action, env_params)
      # Create new Brax State
      nstate = GymnaxState(
          obs=obs,
          reward=reward,
          done=done,
    ...
    first_done = jnp.argmax(state.done)
...
class AutoResetWrapper(Wrapper):
  ...
  def step(self, state: State, action: jax.Array, env_params) -> State:
    ...
    state = self.env.step(state, action, env_params)
...
class EvalWrapper(Wrapper):
  ...
  def step(self, state: State, action: jax.Array, env_params) -> State:
    ...
    key= jax.random.PRNGKey(0)
    nstate = self.env.step(key, state, action, env_params)
...
class DomainRandomizationVmapWrapper(Wrapper):
  ...
  def reset(self, rng: jax.Array, env_params) -> State:
    def reset(sys, rng):
